import { useState } from "react";
import { useDispatch } from "react-redux";
import { Link, generatePath } from "react-router-dom";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";

import { deleteCategory } from "../../../redux/slice/admin/category.slice";
import ROUTES from "../../../constants/ROUTES";

const CategoryItem = ({ item }) => {
  const dispatch = useDispatch();
  const [loading, setLoading] = useState(false);
  const [openChild, setOpenChild] = useState(false);

  const handleDeleteCategory = (categoryId) => {
    setLoading(true);
    dispatch(
      deleteCategory({
        data: categoryId,
        callback: {
          finish: () => setLoading(false),
        },
      })
    );
  };

  const handleRenderChild = () => {
    return item?.children?.map((item) => {
      return <CategoryItem key={item?.id} item={item} />;
    });
  };

  return (
    <div className="w-full flex flex-col justify-start items-center gap-4">
      <div className="w-full flex justify-start items-center gap-4">
        <div className="shrink-0 w-24 h-24 border border-slate-300 rounded p-1">
          <img
            src={item?.imageUrl}
            alt=""
            className="object-cover aspect-square rounded"
          />
        </div>
        <div className="flex-1 flex justify-between items-center">
          <div className="flex-1">
            <span className="text-xl font-medium">{item?.content}</span>
          </div>
          <div className="shrink-0 flex justify-center items-center gap-4">
            <Link
              to={generatePath(ROUTES.ADMIN.CATEGORIES.EDIT, {
                id: item?.id,
              })}
              state={{ categoryId: item?.id }}
              className="w-20 h-10 flex justify-center items-center text-white bg-yellow-500 rounded"
            >
              Edit
            </Link>
            {item?.parentId === 0 ? (
              <div className="group relative">
                <button className="w-20 h-10 text-white bg-slate-300 border border-red-600 rounded">
                  Delete
                </button>
                <div className="absolute hidden top-10 left-0 z-20 p-2 group-hover:block">
                  <div className="w-36 bg-slate-600 p-1 text-white rounded">
                    <p>Category is root</p>
                    <p>Can't delete</p>
                  </div>
                </div>
              </div>
            ) : (
              <button
                onClick={() => handleDeleteCategory(item?.id)}
                className="w-20 h-10 flex justify-center items-center gap-2 text-white bg-red-600 rounded"
              >
                {loading ? (
                  <div>
                    <svg
                      aria-hidden="true"
                      className="inline w-4 h-4 text-gray-200 animate-spin fill-red-600"
                      viewBox="0 0 100 101"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                        fill="currentColor"
                      />
                      <path
                        d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                        fill="currentFill"
                      />
                    </svg>
                  </div>
                ) : (
                  ""
                )}
                <span>Delete</span>
              </button>
            )}
          </div>
        </div>
      </div>
      <div className="w-full flex flex-col justify-start items-center gap-4">
        <div className="w-full flex justify-start items-center">
          {item?.children?.length !== 0 ? (
            <div className="">
              {openChild ? (
                <button
                  onClick={() => setOpenChild(false)}
                  className="flex justify-start items-center gap-2"
                >
                  <FontAwesomeIcon icon="fas fa-arrow-up" />
                  <span>Hide children</span>
                </button>
              ) : (
                <button
                  onClick={() => setOpenChild(true)}
                  className="flex justify-start items-center gap-2"
                >
                  <FontAwesomeIcon icon="fas fa-arrow-down" />
                  <span>Show children</span>
                </button>
              )}
            </div>
          ) : (
            ""
          )}
        </div>
        <div className="w-full pl-8 border-l border-slate-300">
          {openChild && item?.children?.length !== 0 ? (
            <div className="w-full flex flex-col justify-start items-center gap-4">
              {handleRenderChild()}
            </div>
          ) : (
            ""
          )}
        </div>
      </div>
    </div>
  );
};

export default CategoryItem;
