...
import {
  getAllAvailable,
  getVoucher,
} from "../../../redux/slice/user/voucher.user.slice";
...
const ClaimVoucher = () => {
  ...
  const { data, loading } = useSelector(
    (state) => state.VoucherUser.get_all_available
  );
  const { get_voucher } = useSelector((state) => state.VoucherUser);
    ...
    dispatch(
      getVoucher({
        ...
        callback: {
          ...
          goToList: () => navigate(ROUTES.USER.VOUCHER_WALLET.AVAILABLE),
        },
      })
    );
  };
    ...
    return data?.map((voucher) => {
      return (
        <div
          ...
          className="col-span-1 w-full flex flex-col justify-start items-center gap-2 border border-slate-300 rounded p-2"
        >
          <div className="w-full">
            <span className="font-medium">Shop : </span>
            <span>{voucher?.seller?.storeName}</span>
          </div>
          <div className="w-full">
          ...
          <div className="w-full">
            ...
            <span>{voucher?.usageLimitPerUser}</span>
          </div>
  ...
  return (
    <div className="w-full flex flex-col justify-start items-center gap-4">
      <div className="w-full flex justify-center items-center">
        <h2 className="font-semibold text-xl">Available Voucher</h2>
      </div>
      {data?.length !== 0 ? (
