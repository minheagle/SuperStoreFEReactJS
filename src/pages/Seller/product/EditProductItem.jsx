import { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { useParams, useNavigate, generatePath, Link } from "react-router-dom";

import { Formik, Form, Field, FieldArray, ErrorMessage } from "formik";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";

import {
  getProductItemDetail,
  updateProductItem,
} from "../../../redux/slice/seller/product.seller.slice";
import updateProductItemWithoutImageSchema from "../../../utils/validate/Seller/product/update.product.item.schema";
import ROUTES from "../../../constants/ROUTES";

import LoadingFull from "../../../components/common/LoadingFull";
import ListImage from "../../../components/Seller/product/update/ListImage";

const EditProductItem = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { shopName, productId, productItemId } = useParams();

  const { data, loading } = useSelector(
    (state) => state.ProductSeller.get_item_detail
  );
  const { update_product_item } = useSelector((state) => state.ProductSeller);

  useEffect(() => {
    dispatch(getProductItemDetail({ productId, productItemId }));
  }, [productId, productItemId]);

  const initialValues = {
    price: data?.price,
    qtyInStock: data?.qtyInStock,
    // status: data?.status,
    optionTypes: data?.optionTypes,
  };

  const handleCallback = () => {
    dispatch(getProductItemDetail({ productId, productItemId }));
  };

  const handleSubmit = (values) => {
    const optionTypes = values?.optionTypes?.map((item) => {
      return {
        optionName: item.optionName,
        optionValueRequest: {
          valueName: item.optionValue.valueName,
        },
      };
    });
    const itemRequestEdit = {
      price: values.price,
      qtyInStock: values.qtyInStock,
      optionTypes,
    };
    dispatch(
      updateProductItem({
        productItemId,
        itemRequestEdit,
        callback: {
          reload: () => handleCallback(),
        },
      })
    );
  };

  if (loading) {
    return <LoadingFull />;
  }

  return (
    <div className="w-full flex flex-col justify-center items-center gap-4">
      <div className="w-full flex flex-col justify-start items-center">
        <div className="w-full flex justify-center items-center">
          <h1>Edit Product</h1>
        </div>
        <div className="w-full flex justify-start items-center gap-4 pl-8">
          <Link
            to={generatePath(ROUTES.SELLER.PRODUCT.LIST, { shopName })}
            className="flex justify-center items-center gap-2 px-2 py-1 bg-primary text-white rounded"
          >
            Go to list
          </Link>
          <Link
            to={generatePath(ROUTES.SELLER.PRODUCT.UPDATE, {
              shopName,
              productId,
            })}
            className="flex justify-center items-center gap-2 px-2 py-1 bg-primary text-white rounded"
          >
            Go to product
          </Link>
        </div>
      </div>
      <Formik
        initialValues={initialValues}
        validationSchema={updateProductItemWithoutImageSchema}
        onSubmit={(values) => handleSubmit(values)}
      >
        {({ errors, values }) => (
          <Form className="w-3/5 flex flex-col justify-start items-center gap-4 border border-slate-300 rounded p-4">
            {/* Price */}
            <div className="w-full flex flex-col justify-start items-center gap-2">
              <div className="w-full flex justify-center items-center">
                <div className="w-1/2">
                  <span className="font-semibold">Price :</span>
                </div>
                <div className="w-1/2 flex justify-end items-center gap-4 text-red-600">
                  {errors.price ? (
                    <>
                      <FontAwesomeIcon icon="fas fa-info-circle" />
                      <span>{errors.price}</span>
                    </>
                  ) : (
                    ""
                  )}
                </div>
              </div>
              <Field
                name="price"
                className={`w-full outline-none border rounded pl-1 ${
                  errors.price ? "border-red-600" : "border-slate-300"
                }`}
              />
            </div>
            {/* Quantity Stock */}
            <div className="w-full flex flex-col justify-start items-center gap-2">
              <div className="w-full flex justify-center items-center">
                <div className="w-1/2">
                  <span className="font-semibold">Quantity Stock :</span>
                </div>
                <div className="w-1/2 flex justify-end items-center gap-4 text-red-600">
                  {errors.qtyInStock ? (
                    <>
                      <FontAwesomeIcon icon="fas fa-info-circle" />
                      <span>{errors.qtyInStock}</span>
                    </>
                  ) : (
                    ""
                  )}
                </div>
              </div>
              <Field
                name="qtyInStock"
                className={`w-full outline-none border rounded pl-1 ${
                  errors.qtyInStock ? "border-red-600" : "border-slate-300"
                }`}
              />
            </div>
            <div className="w-full flex flex-col justify-start items-center gap-4">
              <div className="w-full flex justify-start items-center">
                <span className="font-semibold">Options :</span>
              </div>
              <FieldArray name="optionTypes">
                {({ push, remove }) => (
                  <div className="w-full flex flex-col justify-start items-center gap-2 pl-8">
                    {values?.optionTypes?.map((option, index) => {
                      return (
                        <div
                          key={index}
                          className="w-full flex justify-start items-center"
                        >
                          <div className="w-1/2 flex justify-start items-center gap-2">
                            <div className="shrink-0 flex justify-start items-center gap-1">
                              <label
                                htmlFor={`optionTypes.${index}.optionValue.valueName`}
                                className="w-24 font-medium"
                              >
                                {option.optionName}
                              </label>
                              <span className="font-medium">:</span>
                            </div>
                            <div className="flex-1 flex justify-center items-center">
                              <Field
                                type="text"
                                id={`optionTypes.${index}.optionValue.valueName`}
                                name={`optionTypes.${index}.optionValue.valueName`}
                                className="w-full outline-none border border-slate-300 pl-2 rounded"
                              />
                            </div>
                          </div>
                          <div className="w-1/2 flex justify-start items-center text-red-600 pl-2">
                            <ErrorMessage
                              name={`optionTypes.${index}.optionValue.valueName`}
                              component="div"
                            />
                          </div>
                        </div>
                      );
                    })}
                  </div>
                )}
              </FieldArray>
            </div>
            <div className="w-full flex justify-start items-center">
              <button
                type="submit"
                className="flex justify-start items-center gap-2 px-2 py-1 bg-primary text-white rounded"
              >
                {update_product_item.loading ? (
                  <div>
                    <svg
                      aria-hidden="true"
                      className="inline w-4 h-4 text-gray-200 animate-spin fill-primary"
                      viewBox="0 0 100 101"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                        fill="currentColor"
                      />
                      <path
                        d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                        fill="currentFill"
                      />
                    </svg>
                  </div>
                ) : (
                  ""
                )}
                <span>Submit</span>
              </button>
            </div>
          </Form>
        )}
      </Formik>
      <div className="w-3/5 flex justify-center items-center border border-slate-300 rounded">
        <ListImage data={data?.imageProductList} />
      </div>
    </div>
  );
};

export default EditProductItem;
